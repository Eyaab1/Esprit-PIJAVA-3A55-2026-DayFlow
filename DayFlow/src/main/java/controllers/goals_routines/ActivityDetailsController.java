...
public class ActivityDetailsController {
    ...
    private void onBackToRoutine() {
        if (currentRoutine == null || currentRoutine.getGoal() == null) {
            return;
        }
        try {
            Goal g = goalService.findById(currentRoutine.getGoal().getId());
            if (g == null) {
                NavigationManager.show("/user/goals_routines/goals_dashboard.fxml", "DayFlow — Mes objectifs");
                return;
            }
            RoutineDetailsController controller = NavigationManager.showAndGetController(
                    "/user/goals_routines/routine-details.fxml", "DayFlow — Routines de l'objectif");
    ...
    private void onDeleteRoutine() {
        ...
        confirm.showAndWait().ifPresent(response -> {
            if (response == ButtonType.OK) {
                try {
                    activityService.deleteByRoutineId(currentRoutine.getId());
