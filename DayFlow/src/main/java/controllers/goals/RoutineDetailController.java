...
public class RoutineDetailController {
    ...
    private VBox buildActivityCard(Activity a) {
        ...
        sep.getStyleClass().add("activity-card-sep");
        ...
        del.getStyleClass().add("activity-btn-del-icon");
        del.setDisable(!canEdit);
        del.setOnAction(ev -> deleteActivity(a));

        foot.getChildren().addAll(edit, del);

        card.getChildren().addAll(head, line1, line2, timerBox, sep, statusBadge, foot);
        return card;
    }
    ...
    private void showActivityDialog(Activity existing) {
        ...
        dialog.setHeaderText(editing ? "Modifier l'activité" : "Nouvelle Activité");
